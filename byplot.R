...
library(ggplot2)
library(reshape2)
## dt = data.table, outcome = main varible you want to count,
## byval = variable you want to categorize 'count' inv = if T,
## change order
byplot <- function(dt, outcome, byval, position = "stack", inv = F, 
  type = "bar", binwidth = 100) {
  ...
  if (type == "bar") {
    ...
    ggdt <- ggplot(subdt, aes_string(x = outcome, y = "N", 
      fill = byval))
